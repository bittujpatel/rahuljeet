...
faqItems.forEach((item) => {
  ...
  item.addEventListener("mouseleave", () => {
    // Delay the removal of the "expanded" class for a smoother transition
    setTimeout(() => {
      item.classList.remove("expanded");
    }, 300); // Adjust the delay duration as needed (300 milliseconds in this example)
  });
});

document.querySelector('.pageclip-form').addEventListener('submit', function(event) {
